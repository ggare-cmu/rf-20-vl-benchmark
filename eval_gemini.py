...
# 60 second timeout for API calls (prevents hanging indefinitely)
...
def create_few_shot_prompt(train_folder, categories_dict):
    ...
    for img_id in annotated_image_ids:
        ...
        # Get only the classes present in this image's annotations
        present_cat_ids = set(ann['category_id'] for ann in example_annotations)
        present_category_names = [categories_dict[cat_id] for cat_id in present_cat_ids if cat_id in categories_dict]
        category_prompt_for_example = ", ".join(present_category_names)

        user_prompt_text_for_example = f"Detect the 2d bounding boxes of the following objects: {category_prompt_for_example}. The box_2d should be [ymin, xmin, ymax, xmax] normalized to 0-1000."
        text_part_for_example = Part(text=user_prompt_text_for_example)

        user_content = Content(role="user", parts=[image_part, text_part_for_example])  # image before text per Gemini best practices
...
def precompute_prompts_for_dataset(dataset_dir, categories, categories_dict, ipt_definitions=None):
    """Precompute prompts, including multi-turn few-shot and IPT-optimized definitions."""
    ...
    basic_system_prompt = "Return bounding boxes as a JSON array with labels. Never return masks."
    final_query_text_basic = f"Detect the 2d bounding boxes of the following objects: {category_prompt}. The box_2d should be [ymin, xmin, ymax, xmax] normalized to 0-1000."

    instructions_system_prompt = basic_system_prompt
    final_query_text_instructions = f"Detect the 2d bounding boxes of the following objects: {category_prompt}. The box_2d should be [ymin, xmin, ymax, xmax] normalized to 0-1000.\n\nUse the following annotator instructions to improve detection accuracy:\n{instructions}\n"
    ...
    # Add IPT prompt if optimized definitions are provided
    if ipt_definitions:
        ipt_instructions = "\n".join([
            f"- {cls}: {defn}" for cls, defn in ipt_definitions.items()
        ])
        final_query_text_ipt = f"Detect the 2d bounding boxes of the following objects: {category_prompt}. The box_2d should be [ymin, xmin, ymax, xmax] normalized to 0-1000.\n\nUse the following class definitions to improve detection accuracy:\n{ipt_instructions}\n"
        prompts['ipt'] = (basic_system_prompt, few_shot_contents_list, final_query_text_ipt, ipt_instructions)
...
def process_image(args):
    ...
    (image_info, image_id, file_name,
     ...
     prompts, few_shot, just_instructions, combined, use_ipt,
    ...
    try:
        ...
        if use_ipt and 'ipt' in prompts: mode = 'ipt'
        elif combined: mode = 'combined'
        ...
        final_user_turn_parts = [final_image_part, final_text_part]  # image before text per Gemini best practices
        ...
        if err:
            ...
            if not boxes_all:
                logger.info(f"Image {image_id} ({file_name}): API call successful, but no valid boxes detected/parsed from response: {response_text[:500]}...")
            else:
    ...
    with status_file_lock:
        ...
        # Save results incrementally per image
        if current_attempt_status and coco_annotations:
            incremental_results_file = os.path.join(results_dir, "gemini_detection_results.json")
            try:
                existing_results = []
                if os.path.exists(incremental_results_file):
                    with open(incremental_results_file, 'r') as f:
                        existing_results = json.load(f)
                existing_results.extend(coco_annotations)
                with open(incremental_results_file, 'w') as f:
                    json.dump(existing_results, f, indent=2)
                logger.info(f"Saved {len(coco_annotations)} annotations for image {image_id}. Total: {len(existing_results)}")
            except Exception as save_e:
                logger.error(f"Failed to save incremental results for {image_id}: {save_e}")
...
def process_dataset(dataset_dir, few_shot, just_instructions, combined, model_id, output_dir_root, visualize_dir_root,
                    use_ipt=False, ipt_definitions=None):
    ...
    prompts = precompute_prompts_for_dataset(dataset_dir, category_names, categories_dict, ipt_definitions)
...
def main():
    ...
    parser.add_argument('--dataset_location', type=str, default="rf100-vl-fsod",
    ...
    parser.add_argument('--ipt', action='store_true',
                        help='Run Iterative Prompt Tuning before evaluation')
    parser.add_argument('--ipt_iterations', type=int, default=3,
                        help='Number of IPT iterations per class')
    parser.add_argument('--ipt_checkpoint_dir', type=str, default='gemini_ipt_test_exp',
                        help='Directory for IPT checkpoints (enables resume)')
    parser.add_argument('--model_id', type=str, default="gemini-3-pro-preview",
                        help='The Gemini model ID to use for inference')
    parser.add_argument('--save_dir', type=str, default="gemini3_test_fsod_test_exp",
    ...
    # Check if dataset_location is a single dataset (has test folder) or a parent folder
    if os.path.exists(os.path.join(args.dataset_location, "test")):
        # Single dataset provided directly
        all_dataset_dirs = [args.dataset_location]
        logger.info(f"Processing single dataset: {args.dataset_location}")
    else:
        # Parent folder containing multiple datasets
    ...
    for dataset_dir in tqdm(all_dataset_dirs, desc="Processing datasets", unit="dataset"):
        ...
        # Run IPT if requested
        ipt_definitions = None
        if args.ipt:
            from eval_gemini_ipt import run_ipt_for_dataset
            logger.info(f"Running IPT for dataset: {dataset_name}")
            ipt_definitions = run_ipt_for_dataset(
                client=client,
                model_id=args.model_id,
                dataset_dir=dataset_dir,
                num_iterations=args.ipt_iterations,
                checkpoint_dir=args.ipt_checkpoint_dir,
                rate_limiter=rate_limiter,
                logger=logger
            )
            logger.info(f"IPT completed for {dataset_name}. Optimized {len(ipt_definitions)} classes.")

            # Save the final assembled IPT prompt (always, even if loaded from checkpoint)
            ipt_prompt_dir = os.path.join(args.ipt_checkpoint_dir, dataset_name)
            os.makedirs(ipt_prompt_dir, exist_ok=True)

            # Build the final prompt text (same format as used in detection)
            category_names = list(ipt_definitions.keys())
            category_prompt = ", ".join(category_names)
            ipt_instructions_text = "\n".join([
                f"- {cls}: {defn}" for cls, defn in ipt_definitions.items()
            ])
            final_ipt_prompt = f"""Detect the 2d bounding boxes of the following objects: {category_prompt}. The box_2d should be [ymin, xmin, ymax, xmax] normalized to 0-1000.

Use the following class definitions to improve detection accuracy:
{ipt_instructions_text}
"""
            final_prompt_path = os.path.join(ipt_prompt_dir, f"final_ipt_prompt_{dataset_name}.txt")
            with open(final_prompt_path, "w", encoding="utf-8") as f:
                f.write(final_ipt_prompt)
            logger.info(f"Saved final IPT prompt to {final_prompt_path}")
        ...
        _, _, processed, errored, skipped = process_dataset(
            ...
            use_ipt=args.ipt, ipt_definitions=ipt_definitions
        )
