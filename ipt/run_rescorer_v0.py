...
def rescore_dataset(args, dataset_path, 
    ...
    combined_detections = []
    for det_model, det_ranking in zip(detections_all_by_type["model"], detections_all_by_type["ranking"]):
        
        # Check every thing matches except score
        if det_model["image_id"] != det_ranking["image_id"] or \
            det_model["category_id"] != det_ranking["category_id"] or \
                det_model["bbox"] != det_ranking["bbox"]:
            raise ValueError(f"Warning: Mismatched detections in loaded predictions for {dataset_path}. Model: {det_model}. Ranking: {det_ranking}")
        
        #Combine the scores
        score = det_model["score"] * det_ranking["score"]
        # score = (det_model["score"] + det_ranking["score"]) / 2.0
        # score = det_model["score"] + 0.01*det_ranking["score"]

        combined_detections.append({
            "image_id": det_model["image_id"],
            "category_id": det_model["category_id"],
            "bbox": det_model["bbox"],
            "score": score,
            # "score_combined": score,    
            # "score_model": det_model["score"],
            # "score_ranking": det_ranking["score"],
        })
