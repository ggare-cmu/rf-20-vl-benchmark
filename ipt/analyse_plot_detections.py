...
# Initialise index on first load or when dataset changes
...
# Prev / Next buttons update the index BEFORE the selectbox is rendered
...
with col_nav3:
    if st.button("Next ▶", use_container_width=True):
        st.session_state["img_idx"] = min(n_imgs - 1, st.session_state["img_idx"] + 1)

# Selectbox is the single source of truth — it reads AND writes img_idx
with col_nav2:
    st.session_state["img_idx"] = st.selectbox(
        "Select image",
        range(n_imgs),
